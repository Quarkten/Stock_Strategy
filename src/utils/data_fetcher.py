...
class DataFetcher:
    ...
    def fetch_data(self, symbol: str, timeframe: str, limit: int | None = None, provider: str | None = None, period: str | None = None, **kwargs):
        ...
        try:
            if hasattr(prov, "get_historical_data"):
                df = prov.get_historical_data(symbol, interval=interval, period=period, **kwargs)
            else:
                logger.error(f"Selected provider '{prov_name}' lacks get_historical_data()")
                return pd.DataFrame()
        except Exception as e:
            logger.error(f"Error in provider '{prov_name}' get_historical_data: {e}")
            return pd.DataFrame()
